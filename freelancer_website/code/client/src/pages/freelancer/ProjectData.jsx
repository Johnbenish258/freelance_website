...
const ProjectData = () => {
  ...
  const fetchProject = async(id) =>{
    await axios.get(`/fetch-project/${id}`).then(
      (response)=>{
    ...
    const handleBidding = async() =>{
      ...
      await axios.post("/make-bid", {clientId, freelancerId, projectId, proposal, bidAmount, estimatedTime}).then(
        (response)=>{
            setProposal('');
            setBidAmount(0);
            setEstimatedTime(0);
    ...
    const handleProjectSubmission = async() =>{
      ...
      await axios.post("/submit-project", {clientId, freelancerId, projectId, projectLink, manualLink, submissionDescription}).then(
        (response)=>{
            setProjectLink('');
            setManualLink('');
    ...
    const fetchChats = async() =>{
      await axios.get(`/fetch-chats/${params['id']}`).then(
        (response) =>{
